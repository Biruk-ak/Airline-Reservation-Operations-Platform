/**
 * Utility helpers for Reporting module UI logic.
 */

export function normalizeReportingFilters(raw = {}) {
  const out = {};
...
export function sortReportingRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupReportingByStation(rows) {
...
export function groupReportingByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeReportingPriorityScore(row) {
...
export function filterReportingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeReportingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildReportingExportFilename(prefix = 'reporting') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateReportingForm(form) {
  const errors = {};
...
export function mergeReportingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeReportingEntity(row) {
