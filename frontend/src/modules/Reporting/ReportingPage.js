...
export default function ReportingPage() {
  ...
  const createEntity = async (e) => {
    ...
    try {
      ...
      dispatch(pushToast({ type: 'success', message: 'ReportDefinition created' }));
      setCreating(false);
  ...
  return (
    <div className="reporting-page">
      ...
      <p className="arop-page-sub">
        Manage reporting records, operational status, priorities, and station-level workflows.
      </p>
      ...
      <div className="arop-toolbar">
        ...
        <button className="arop-btn secondary" type="button" onClick={() => setCreating((v) => !v)}>
          {creating ? 'Cancel' : 'New ReportDefinition'}
        </button>
      ...
      {creating && (
        <form onSubmit={createEntity} style={{ marginBottom: '1rem', display: 'grid', gap: '0.6rem', maxWidth: 720 }}>
          ...
          <button className="arop-btn" type="submit" disabled={busy}>Create ReportDefinition</button>
        </form>
      )}
      ...
      <div className="arop-table-wrap">
        <table className="arop-table">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={10}>No reporting records found.</td></tr>
            )}
            {rows.map((row) => (
      ...
      <EntityDetailDrawer
        open={Boolean(detail)}
        title={detail?.summary?.name || 'ReportDefinition detail'}
        data={detail}
