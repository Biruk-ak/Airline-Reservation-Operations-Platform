/**
 * Utility helpers for Cargo module UI logic.
 */

export function normalizeCargoFilters(raw = {}) {
  const out = {};
...
export function sortCargoRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupCargoByStation(rows) {
...
export function groupCargoByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeCargoPriorityScore(row) {
...
export function filterCargoHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeCargoPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildCargoExportFilename(prefix = 'cargo') {
...
export function mergeCargoMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeCargoEntity(row) {
