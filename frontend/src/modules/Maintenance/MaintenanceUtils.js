/**
 * Utility helpers for Maintenance module UI logic.
 */

export function normalizeMaintenanceFilters(raw = {}) {
  const out = {};
...
export function sortMaintenanceRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupMaintenanceByStation(rows) {
...
export function groupMaintenanceByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeMaintenancePriorityScore(row) {
...
export function filterMaintenanceHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeMaintenancePriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildMaintenanceExportFilename(prefix = 'maintenance') {
...
export function mergeMaintenanceMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeMaintenanceEntity(row) {
