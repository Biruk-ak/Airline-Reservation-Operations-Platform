...
const MODULE_KEY = 'flights';
const PAGE_TITLE = 'Flight Scheduling';
...
export default function FlightSchedulingPage() {
  ...
  const createEntity = async (e) => {
    ...
    try {
      ...
      dispatch(pushToast({ type: 'success', message: 'Flight created' }));
      setCreating(false);
  ...
  const exportRows = async () => {
    ...
    a.href = url;
    a.download = `flights-export.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  return (
    <div className="flightscheduling-page">
      <h1 className="arop-page-title">{PAGE_TITLE}</h1>
      <p className="arop-page-sub">
        Manage flight scheduling records, operational status, priorities, and station-level workflows.
      </p>
      ...
      {creating && (
        <form onSubmit={createEntity} style={{ marginBottom: '1rem', display: 'grid', gap: '0.6rem', maxWidth: 720 }}>
          ...
          <button className="arop-btn" type="submit" disabled={busy}>Create Flight</button>
        </form>
      )}
      ...
      <EntityDetailDrawer
        open={Boolean(detail)}
        title={detail?.summary?.name || 'Flight detail'}
        data={detail}
