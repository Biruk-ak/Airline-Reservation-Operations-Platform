/**
 * Utility helpers for FlightScheduling module UI logic.
 */

export function normalizeFlightSchedulingFilters(raw = {}) {
  const out = {};
...
export function sortFlightSchedulingRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupFlightSchedulingByStation(rows) {
...
export function groupFlightSchedulingByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeFlightSchedulingPriorityScore(row) {
...
export function filterFlightSchedulingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeFlightSchedulingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildFlightSchedulingExportFilename(prefix = 'flightscheduling') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateFlightSchedulingForm(form) {
  const errors = {};
...
export function mergeFlightSchedulingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeFlightSchedulingEntity(row) {
