/**
 * Utility helpers for LoyaltyProgram module UI logic.
 */

export function normalizeLoyaltyProgramFilters(raw = {}) {
  const out = {};
...
export function sortLoyaltyProgramRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupLoyaltyProgramByStation(rows) {
...
export function groupLoyaltyProgramByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeLoyaltyProgramPriorityScore(row) {
...
export function filterLoyaltyProgramHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeLoyaltyProgramPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildLoyaltyProgramExportFilename(prefix = 'loyaltyprogram') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateLoyaltyProgramForm(form) {
  const errors = {};
...
export function mergeLoyaltyProgramMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeLoyaltyProgramEntity(row) {
