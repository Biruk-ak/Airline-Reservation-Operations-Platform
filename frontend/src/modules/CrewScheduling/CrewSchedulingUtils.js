/**
 * Utility helpers for CrewScheduling module UI logic.
 */

export function normalizeCrewSchedulingFilters(raw = {}) {
  const out = {};
...
export function sortCrewSchedulingRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupCrewSchedulingByStation(rows) {
...
export function groupCrewSchedulingByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeCrewSchedulingPriorityScore(row) {
...
export function filterCrewSchedulingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeCrewSchedulingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildCrewSchedulingExportFilename(prefix = 'crewscheduling') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateCrewSchedulingForm(form) {
  const errors = {};
...
export function mergeCrewSchedulingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeCrewSchedulingEntity(row) {
