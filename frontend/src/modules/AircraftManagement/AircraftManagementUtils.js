/**
 * Utility helpers for AircraftManagement module UI logic.
 */

export function normalizeAircraftManagementFilters(raw = {}) {
  const out = {};
...
export function sortAircraftManagementRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupAircraftManagementByStation(rows) {
...
export function groupAircraftManagementByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeAircraftManagementPriorityScore(row) {
...
export function filterAircraftManagementHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeAircraftManagementPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildAircraftManagementExportFilename(prefix = 'aircraftmanagement') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateAircraftManagementForm(form) {
  const errors = {};
...
export function mergeAircraftManagementMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeAircraftManagementEntity(row) {
