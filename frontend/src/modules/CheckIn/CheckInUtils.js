/**
 * Utility helpers for CheckIn module UI logic.
 */

export function normalizeCheckInFilters(raw = {}) {
  const out = {};
...
export function sortCheckInRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupCheckInByStation(rows) {
...
export function groupCheckInByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeCheckInPriorityScore(row) {
...
export function filterCheckInHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeCheckInPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildCheckInExportFilename(prefix = 'checkin') {
...
export function mergeCheckInMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeCheckInEntity(row) {
