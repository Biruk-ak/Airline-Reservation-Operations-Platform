...
export default function CheckInPage() {
  ...
  const createEntity = async (e) => {
    ...
    try {
      ...
      dispatch(pushToast({ type: 'success', message: 'CheckIn created' }));
      setCreating(false);
  ...
  const exportRows = async () => {
    ...
    a.href = url;
    a.download = `checkins-export.json`;
    a.click();
  ...
  return (
    <div className="checkin-page">
      ...
      <p className="arop-page-sub">
        Manage check-in records, operational status, priorities, and station-level workflows.
      </p>
      ...
      {creating && (
        <form onSubmit={createEntity} style={{ marginBottom: '1rem', display: 'grid', gap: '0.6rem', maxWidth: 720 }}>
          ...
          <button className="arop-btn" type="submit" disabled={busy}>Create CheckIn</button>
        </form>
      )}
      ...
      <div className="arop-table-wrap">
        <table className="arop-table">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={10}>No check-in records found.</td></tr>
            )}
            {rows.map((row) => (
      ...
      <EntityDetailDrawer
        open={Boolean(detail)}
        title={detail?.summary?.name || 'CheckIn detail'}
        data={detail}
