/**
 * Utility helpers for SeatSelection module UI logic.
 */

export function normalizeSeatSelectionFilters(raw = {}) {
  const out = {};
...
export function sortSeatSelectionRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupSeatSelectionByStation(rows) {
...
export function groupSeatSelectionByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeSeatSelectionPriorityScore(row) {
...
export function filterSeatSelectionHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeSeatSelectionPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildSeatSelectionExportFilename(prefix = 'seatselection') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateSeatSelectionForm(form) {
  const errors = {};
...
export function mergeSeatSelectionMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeSeatSelectionEntity(row) {
