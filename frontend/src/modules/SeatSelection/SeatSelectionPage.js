...
const MODULE_KEY = 'seats';
const PAGE_TITLE = 'Seat Selection';
...
export default function SeatSelectionPage() {
  ...
  const createEntity = async (e) => {
    ...
    try {
      ...
      dispatch(pushToast({ type: 'success', message: 'SeatMap created' }));
      setCreating(false);
  ...
  const exportRows = async () => {
    ...
    a.href = url;
    a.download = `seats-export.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  return (
    <div className="seatselection-page">
      <h1 className="arop-page-title">{PAGE_TITLE}</h1>
      <p className="arop-page-sub">
        Manage seat selection records, operational status, priorities, and station-level workflows.
      </p>
      ...
      {creating && (
        <form onSubmit={createEntity} style={{ marginBottom: '1rem', display: 'grid', gap: '0.6rem', maxWidth: 720 }}>
          ...
          <button className="arop-btn" type="submit" disabled={busy}>Create SeatMap</button>
        </form>
      )}
      ...
      <div className="arop-table-wrap">
        <table className="arop-table">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={10}>No seat selection records found.</td></tr>
            )}
            {rows.map((row) => (
      ...
      <EntityDetailDrawer
        open={Boolean(detail)}
        title={detail?.summary?.name || 'SeatMap detail'}
        data={detail}
