/**
 * Utility helpers for AdminPortal module UI logic.
 */

export function normalizeAdminPortalFilters(raw = {}) {
  const out = {};
...
export function sortAdminPortalRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupAdminPortalByStation(rows) {
...
export function groupAdminPortalByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeAdminPortalPriorityScore(row) {
...
export function filterAdminPortalHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeAdminPortalPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildAdminPortalExportFilename(prefix = 'adminportal') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateAdminPortalForm(form) {
  const errors = {};
...
export function mergeAdminPortalMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeAdminPortalEntity(row) {
