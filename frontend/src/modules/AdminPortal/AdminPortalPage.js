...
export default function AdminPortalPage() {
  ...
  const createEntity = async (e) => {
    ...
    try {
      ...
      dispatch(pushToast({ type: 'success', message: 'AdminAuditLog created' }));
      setCreating(false);
  ...
  return (
    <div className="adminportal-page">
      ...
      <p className="arop-page-sub">
        Manage admin portal records, operational status, priorities, and station-level workflows.
      </p>
      ...
      <div className="arop-toolbar">
        ...
        <button className="arop-btn secondary" type="button" onClick={() => setCreating((v) => !v)}>
          {creating ? 'Cancel' : 'New AdminAuditLog'}
        </button>
      ...
      {creating && (
        <form onSubmit={createEntity} style={{ marginBottom: '1rem', display: 'grid', gap: '0.6rem', maxWidth: 720 }}>
          ...
          <button className="arop-btn" type="submit" disabled={busy}>Create AdminAuditLog</button>
        </form>
      )}
      ...
      <div className="arop-table-wrap">
        <table className="arop-table">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={10}>No admin portal records found.</td></tr>
            )}
            {rows.map((row) => (
      ...
      <EntityDetailDrawer
        open={Boolean(detail)}
        title={detail?.summary?.name || 'AdminAuditLog detail'}
        data={detail}
