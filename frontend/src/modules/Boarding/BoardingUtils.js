/**
 * Utility helpers for Boarding module UI logic.
 */

export function normalizeBoardingFilters(raw = {}) {
  const out = {};
...
export function sortBoardingRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupBoardingByStation(rows) {
...
export function groupBoardingByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeBoardingPriorityScore(row) {
...
export function filterBoardingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeBoardingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildBoardingExportFilename(prefix = 'boarding') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateBoardingForm(form) {
  const errors = {};
...
export function mergeBoardingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeBoardingEntity(row) {
