/**
 * Utility helpers for BaggageTracking module UI logic.
 */

export function normalizeBaggageTrackingFilters(raw = {}) {
  const out = {};
...
export function sortBaggageTrackingRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupBaggageTrackingByStation(rows) {
...
export function groupBaggageTrackingByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeBaggageTrackingPriorityScore(row) {
...
export function filterBaggageTrackingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeBaggageTrackingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildBaggageTrackingExportFilename(prefix = 'baggagetracking') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateBaggageTrackingForm(form) {
  const errors = {};
...
export function mergeBaggageTrackingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeBaggageTrackingEntity(row) {
