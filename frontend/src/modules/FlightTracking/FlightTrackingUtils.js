/**
 * Utility helpers for FlightTracking module UI logic.
 */

export function normalizeFlightTrackingFilters(raw = {}) {
  const out = {};
  const keys = ['status', 'station_code', 'region', 'code', 'name', 'q', 'sort', 'dir', 'priority_min'];
  keys.forEach((k) => {
    if (raw[k] !== undefined && raw[k] !== null && raw[k] !== '') out[k] = raw[k];
  });
  if (raw.is_active === true || raw.is_active === false) out.is_active = raw.is_active;
  return out;
}

export function sortFlightTrackingRows(rows, sortKey = 'updated_at', dir = 'desc') {
  const sorted = [...rows].sort((a, b) => {
    const av = a[sortKey];
    const bv = b[sortKey];
    if (av === bv) return 0;
    if (av == null) return 1;
    if (bv == null) return -1;
    if (typeof av === 'number' && typeof bv === 'number') return av - bv;
    return String(av).localeCompare(String(bv));
  });
  return dir === 'asc' ? sorted : sorted.reverse();
}

export function groupFlightTrackingByStation(rows) {
  return rows.reduce((acc, row) => {
    const key = row.station_code || 'UNASSIGNED';
    if (!acc[key]) acc[key] = [];
    acc[key].push(row);
    return acc;
  }, {});
}

export function groupFlightTrackingByStatus(rows) {
  return rows.reduce((acc, row) => {
    const key = row.status || 'unknown';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});
}

export function computeFlightTrackingPriorityScore(row) {
  const base = Number(row.priority || 0);
  const activeBoost = row.is_active ? 2 : 0;
  const statusBoost = {
    in_progress: 3,
    pending_review: 2,
    scheduled: 1,
    draft: 0,
    completed: -1,
    cancelled: -2,
    archived: -3,
  }[row.status] || 0;
  return base + activeBoost + statusBoost;
}

export function filterFlightTrackingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeFlightTrackingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildFlightTrackingExportFilename(prefix = 'flighttracking') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateFlightTrackingForm(form) {
  const errors = {};
  if (!form.name || String(form.name).trim().length < 2) {
    errors.name = 'Name must be at least 2 characters';
  }
  if (form.priority != null) {
    const p = Number(form.priority);
    if (Number.isNaN(p) || p < 0 || p > 10) errors.priority = 'Priority must be 0-10';
  }
  if (form.station_code && !/^[A-Z0-9]{3,4}$/i.test(form.station_code)) {
    errors.station_code = 'Station code must be 3-4 alphanumeric characters';
  }
  return errors;
}

export function mergeFlightTrackingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
    _updated_at: new Date().toISOString(),
  };
}

export function describeFlightTrackingEntity(row) {
  if (!row) return 'No record selected';
  const parts = [row.code, row.name, row.station_code, row.status].filter(Boolean);
  return parts.join(' · ');
}
