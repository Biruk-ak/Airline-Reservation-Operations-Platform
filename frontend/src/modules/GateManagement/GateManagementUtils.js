/**
 * Utility helpers for GateManagement module UI logic.
 */

export function normalizeGateManagementFilters(raw = {}) {
  const out = {};
...
export function sortGateManagementRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupGateManagementByStation(rows) {
...
export function groupGateManagementByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeGateManagementPriorityScore(row) {
...
export function filterGateManagementHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeGateManagementPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildGateManagementExportFilename(prefix = 'gatemanagement') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateGateManagementForm(form) {
  const errors = {};
...
export function mergeGateManagementMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeGateManagementEntity(row) {
