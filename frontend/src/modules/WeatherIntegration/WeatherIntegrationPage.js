...
const MODULE_KEY = 'weather';
const PAGE_TITLE = 'Weather Integration';
...
export default function WeatherIntegrationPage() {
  ...
  const createEntity = async (e) => {
    ...
    try {
      ...
      dispatch(pushToast({ type: 'success', message: 'WeatherObservation created' }));
      setCreating(false);
  ...
  const exportRows = async () => {
    ...
    a.href = url;
    a.download = `weather-export.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  return (
    <div className="weatherintegration-page">
      <h1 className="arop-page-title">{PAGE_TITLE}</h1>
      <p className="arop-page-sub">
        Manage weather integration records, operational status, priorities, and station-level workflows.
      </p>
      ...
      <div className="arop-toolbar">
        ...
        <button className="arop-btn secondary" type="button" onClick={() => setCreating((v) => !v)}>
          {creating ? 'Cancel' : 'New WeatherObservation'}
        </button>
      ...
      {creating && (
        <form onSubmit={createEntity} style={{ marginBottom: '1rem', display: 'grid', gap: '0.6rem', maxWidth: 720 }}>
          ...
          <button className="arop-btn" type="submit" disabled={busy}>Create WeatherObservation</button>
        </form>
      )}
      ...
      <div className="arop-table-wrap">
        <table className="arop-table">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={10}>No weather integration records found.</td></tr>
            )}
            {rows.map((row) => (
      ...
      <EntityDetailDrawer
        open={Boolean(detail)}
        title={detail?.summary?.name || 'WeatherObservation detail'}
        data={detail}
