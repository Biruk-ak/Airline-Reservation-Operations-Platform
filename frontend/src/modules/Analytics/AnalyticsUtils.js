/**
 * Utility helpers for Analytics module UI logic.
 */
...
export function sortAnalyticsRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupAnalyticsByStation(rows) {
...
export function groupAnalyticsByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeAnalyticsPriorityScore(row) {
...
export function filterAnalyticsHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeAnalyticsPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildAnalyticsExportFilename(prefix = 'analytics') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateAnalyticsForm(form) {
  const errors = {};
...
export function mergeAnalyticsMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeAnalyticsEntity(row) {
