/**
 * Utility helpers for Payments module UI logic.
 */

export function normalizePaymentsFilters(raw = {}) {
  const out = {};
...
export function sortPaymentsRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupPaymentsByStation(rows) {
...
export function groupPaymentsByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computePaymentsPriorityScore(row) {
...
export function filterPaymentsHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computePaymentsPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildPaymentsExportFilename(prefix = 'payments') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validatePaymentsForm(form) {
  const errors = {};
...
export function mergePaymentsMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describePaymentsEntity(row) {
