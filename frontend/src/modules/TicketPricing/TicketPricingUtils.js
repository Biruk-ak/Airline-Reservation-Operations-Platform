/**
 * Utility helpers for TicketPricing module UI logic.
 */

export function normalizeTicketPricingFilters(raw = {}) {
  const out = {};
...
export function sortTicketPricingRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupTicketPricingByStation(rows) {
...
export function groupTicketPricingByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeTicketPricingPriorityScore(row) {
...
export function filterTicketPricingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeTicketPricingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildTicketPricingExportFilename(prefix = 'ticketpricing') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateTicketPricingForm(form) {
  const errors = {};
...
export function mergeTicketPricingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeTicketPricingEntity(row) {
