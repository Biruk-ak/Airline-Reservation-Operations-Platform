/**
 * Utility helpers for PassengerBooking module UI logic.
 */

export function normalizePassengerBookingFilters(raw = {}) {
  const out = {};
...
export function sortPassengerBookingRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupPassengerBookingByStation(rows) {
...
export function groupPassengerBookingByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computePassengerBookingPriorityScore(row) {
...
export function filterPassengerBookingHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computePassengerBookingPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildPassengerBookingExportFilename(prefix = 'passengerbooking') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validatePassengerBookingForm(form) {
  const errors = {};
...
export function mergePassengerBookingMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describePassengerBookingEntity(row) {
