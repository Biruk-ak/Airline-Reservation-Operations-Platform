...
const MODULE_KEY = 'airports';
const PAGE_TITLE = 'Airport Operations';
...
export default function AirportOperationsPage() {
  ...
  const createEntity = async (e) => {
    ...
    try {
      ...
      dispatch(pushToast({ type: 'success', message: 'Airport created' }));
      setCreating(false);
  ...
  const exportRows = async () => {
    ...
    a.href = url;
    a.download = `airports-export.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  return (
    <div className="airportoperations-page">
      <h1 className="arop-page-title">{PAGE_TITLE}</h1>
      <p className="arop-page-sub">
        Manage airport operations records, operational status, priorities, and station-level workflows.
      </p>
      ...
      {creating && (
        <form onSubmit={createEntity} style={{ marginBottom: '1rem', display: 'grid', gap: '0.6rem', maxWidth: 720 }}>
          ...
          <button className="arop-btn" type="submit" disabled={busy}>Create Airport</button>
        </form>
      )}
      ...
      <div className="arop-table-wrap">
        <table className="arop-table">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={10}>No airport operations records found.</td></tr>
            )}
            {rows.map((row) => (
      ...
      <EntityDetailDrawer
        open={Boolean(detail)}
        title={detail?.summary?.name || 'Airport detail'}
        data={detail}
