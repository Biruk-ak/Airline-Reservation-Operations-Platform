/**
 * Utility helpers for AirportOperations module UI logic.
 */

export function normalizeAirportOperationsFilters(raw = {}) {
  const out = {};
...
export function sortAirportOperationsRows(rows, sortKey = 'updated_at', dir = 'desc') {
...
export function groupAirportOperationsByStation(rows) {
...
export function groupAirportOperationsByStatus(rows) {
  return rows.reduce((acc, row) => {
...
export function computeAirportOperationsPriorityScore(row) {
...
export function filterAirportOperationsHighAttention(rows) {
  return rows
    .map((r) => ({ ...r, _score: computeAirportOperationsPriorityScore(r) }))
    .filter((r) => r._score >= 8 || r.status === 'pending_review')
    .sort((a, b) => b._score - a._score);
}

export function buildAirportOperationsExportFilename(prefix = 'airportoperations') {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  return `${prefix}-export-${stamp}.json`;
}

export function validateAirportOperationsForm(form) {
  const errors = {};
...
export function mergeAirportOperationsMetadata(existing = {}, patch = {}) {
  return {
    ...existing,
    ...patch,
...
export function describeAirportOperationsEntity(row) {
