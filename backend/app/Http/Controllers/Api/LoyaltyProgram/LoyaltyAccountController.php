...
namespace App\Http\Controllers\Api\LoyaltyProgram;

use App\Http\Controllers\Controller;
use App\Models\LoyaltyProgram\LoyaltyAccount;
use App\Services\LoyaltyProgram\LoyaltyAccountService;
use App\Http\Requests\LoyaltyProgram\StoreLoyaltyAccountRequest;
use App\Http\Requests\LoyaltyProgram\UpdateLoyaltyAccountRequest;
use App\Http\Requests\LoyaltyProgram\SearchLoyaltyAccountRequest;
use App\Http\Requests\LoyaltyProgram\BulkLoyaltyAccountRequest;
...
{
    public function __construct(private LoyaltyAccountService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchLoyaltyAccountRequest $request): JsonResponse
    {
    ...
    public function store(StoreLoyaltyAccountRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'LoyaltyAccount created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'LoyaltyAccount not found'], 404);
        }
    ...
    public function update(UpdateLoyaltyAccountRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'LoyaltyAccount not found'], 404);
        ...
        return response()->json([
            'message' => 'LoyaltyAccount updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'LoyaltyAccount deleted successfully']);
    }
    ...
    public function bulk(BulkLoyaltyAccountRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('LoyaltyAccount external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchLoyaltyAccountRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
