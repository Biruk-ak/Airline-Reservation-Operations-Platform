...
namespace App\Http\Controllers\Api\WeatherIntegration;

use App\Http\Controllers\Controller;
use App\Models\WeatherIntegration\WeatherObservation;
use App\Services\WeatherIntegration\WeatherObservationService;
use App\Http\Requests\WeatherIntegration\StoreWeatherObservationRequest;
use App\Http\Requests\WeatherIntegration\UpdateWeatherObservationRequest;
use App\Http\Requests\WeatherIntegration\SearchWeatherObservationRequest;
use App\Http\Requests\WeatherIntegration\BulkWeatherObservationRequest;
...
{
    public function __construct(private WeatherObservationService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchWeatherObservationRequest $request): JsonResponse
    {
    ...
    public function store(StoreWeatherObservationRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'WeatherObservation created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'WeatherObservation not found'], 404);
        }
    ...
    public function update(UpdateWeatherObservationRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'WeatherObservation not found'], 404);
        ...
        return response()->json([
            'message' => 'WeatherObservation updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'WeatherObservation deleted successfully']);
    }
    ...
    public function bulk(BulkWeatherObservationRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('WeatherObservation external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchWeatherObservationRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
