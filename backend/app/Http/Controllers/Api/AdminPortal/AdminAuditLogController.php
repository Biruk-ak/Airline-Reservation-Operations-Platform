...
use App\Services\AdminPortal\AdminAuditLogService;
use App\Http\Requests\AdminPortal\StoreAdminAuditLogRequest;
use App\Http\Requests\AdminPortal\UpdateAdminAuditLogRequest;
use App\Http\Requests\AdminPortal\SearchAdminAuditLogRequest;
use App\Http\Requests\AdminPortal\BulkAdminAuditLogRequest;
...
{
    public function __construct(private AdminAuditLogService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchAdminAuditLogRequest $request): JsonResponse
    {
    ...
    public function store(StoreAdminAuditLogRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'AdminAuditLog created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'AdminAuditLog not found'], 404);
        }
    ...
    public function update(UpdateAdminAuditLogRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'AdminAuditLog not found'], 404);
        ...
        return response()->json([
            'message' => 'AdminAuditLog updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'AdminAuditLog deleted successfully']);
    }
    ...
    public function bulk(BulkAdminAuditLogRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('AdminAuditLog external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchAdminAuditLogRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
