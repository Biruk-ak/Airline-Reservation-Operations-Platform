...
namespace App\Http\Controllers\Api\FlightTracking;

use App\Http\Controllers\Controller;
use App\Models\FlightTracking\FlightPosition;
use App\Services\FlightTracking\FlightPositionService;
use App\Http\Requests\FlightTracking\StoreFlightPositionRequest;
use App\Http\Requests\FlightTracking\UpdateFlightPositionRequest;
use App\Http\Requests\FlightTracking\SearchFlightPositionRequest;
use App\Http\Requests\FlightTracking\BulkFlightPositionRequest;
...
{
    public function __construct(private FlightPositionService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchFlightPositionRequest $request): JsonResponse
    {
    ...
    public function store(StoreFlightPositionRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'FlightPosition created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'FlightPosition not found'], 404);
        }
    ...
    public function update(UpdateFlightPositionRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'FlightPosition not found'], 404);
        ...
        return response()->json([
            'message' => 'FlightPosition updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'FlightPosition deleted successfully']);
    }
    ...
    public function bulk(BulkFlightPositionRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('FlightPosition external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchFlightPositionRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
