...
use App\Services\Analytics\AnalyticsMetricService;
use App\Http\Requests\Analytics\StoreAnalyticsMetricRequest;
use App\Http\Requests\Analytics\UpdateAnalyticsMetricRequest;
use App\Http\Requests\Analytics\SearchAnalyticsMetricRequest;
use App\Http\Requests\Analytics\BulkAnalyticsMetricRequest;
...
{
    public function __construct(private AnalyticsMetricService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchAnalyticsMetricRequest $request): JsonResponse
    {
    ...
    public function store(StoreAnalyticsMetricRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'AnalyticsMetric created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'AnalyticsMetric not found'], 404);
        }
    ...
    public function update(UpdateAnalyticsMetricRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'AnalyticsMetric not found'], 404);
        ...
        return response()->json([
            'message' => 'AnalyticsMetric updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'AnalyticsMetric deleted successfully']);
    }
    ...
    public function bulk(BulkAnalyticsMetricRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('AnalyticsMetric external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchAnalyticsMetricRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
