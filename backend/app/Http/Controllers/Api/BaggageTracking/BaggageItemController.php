...
namespace App\Http\Controllers\Api\BaggageTracking;

use App\Http\Controllers\Controller;
use App\Models\BaggageTracking\BaggageItem;
use App\Services\BaggageTracking\BaggageItemService;
use App\Http\Requests\BaggageTracking\StoreBaggageItemRequest;
use App\Http\Requests\BaggageTracking\UpdateBaggageItemRequest;
use App\Http\Requests\BaggageTracking\SearchBaggageItemRequest;
use App\Http\Requests\BaggageTracking\BulkBaggageItemRequest;
...
{
    public function __construct(private BaggageItemService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchBaggageItemRequest $request): JsonResponse
    {
    ...
    public function store(StoreBaggageItemRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'BaggageItem created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'BaggageItem not found'], 404);
        }
    ...
    public function update(UpdateBaggageItemRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'BaggageItem not found'], 404);
        ...
        return response()->json([
            'message' => 'BaggageItem updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'BaggageItem deleted successfully']);
    }
    ...
    public function bulk(BulkBaggageItemRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('BaggageItem external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchBaggageItemRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
