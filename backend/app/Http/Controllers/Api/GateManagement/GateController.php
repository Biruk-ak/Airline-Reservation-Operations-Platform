...
namespace App\Http\Controllers\Api\GateManagement;

use App\Http\Controllers\Controller;
use App\Models\GateManagement\Gate;
use App\Services\GateManagement\GateService;
use App\Http\Requests\GateManagement\StoreGateRequest;
use App\Http\Requests\GateManagement\UpdateGateRequest;
use App\Http\Requests\GateManagement\SearchGateRequest;
use App\Http\Requests\GateManagement\BulkGateRequest;
...
{
    public function __construct(private GateService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchGateRequest $request): JsonResponse
    {
    ...
    public function update(UpdateGateRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'Gate not found'], 404);
    ...
    public function bulk(BulkGateRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('Gate external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchGateRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
