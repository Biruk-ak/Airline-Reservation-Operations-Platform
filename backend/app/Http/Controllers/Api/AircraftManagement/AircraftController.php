...
namespace App\Http\Controllers\Api\AircraftManagement;

use App\Http\Controllers\Controller;
use App\Models\AircraftManagement\Aircraft;
use App\Services\AircraftManagement\AircraftService;
use App\Http\Requests\AircraftManagement\StoreAircraftRequest;
use App\Http\Requests\AircraftManagement\UpdateAircraftRequest;
use App\Http\Requests\AircraftManagement\SearchAircraftRequest;
use App\Http\Requests\AircraftManagement\BulkAircraftRequest;
...
{
    public function __construct(private AircraftService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchAircraftRequest $request): JsonResponse
    {
    ...
    public function store(StoreAircraftRequest $request): JsonResponse
    {
    ...
    public function update(UpdateAircraftRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'Aircraft not found'], 404);
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'Aircraft deleted successfully']);
    }
    ...
    public function bulk(BulkAircraftRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('Aircraft external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchAircraftRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
