...
namespace App\Http\Controllers\Api\TicketPricing;

use App\Http\Controllers\Controller;
use App\Models\TicketPricing\FareRule;
use App\Services\TicketPricing\FareRuleService;
use App\Http\Requests\TicketPricing\StoreFareRuleRequest;
use App\Http\Requests\TicketPricing\UpdateFareRuleRequest;
use App\Http\Requests\TicketPricing\SearchFareRuleRequest;
use App\Http\Requests\TicketPricing\BulkFareRuleRequest;
...
{
    public function __construct(private FareRuleService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchFareRuleRequest $request): JsonResponse
    {
    ...
    public function store(StoreFareRuleRequest $request): JsonResponse
    {
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'FareRule not found'], 404);
        }
    ...
    public function update(UpdateFareRuleRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'FareRule not found'], 404);
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'FareRule deleted successfully']);
    }
    ...
    public function bulk(BulkFareRuleRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('FareRule external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchFareRuleRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
