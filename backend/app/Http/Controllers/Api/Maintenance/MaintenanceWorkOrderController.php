...
use App\Services\Maintenance\MaintenanceWorkOrderService;
use App\Http\Requests\Maintenance\StoreMaintenanceWorkOrderRequest;
use App\Http\Requests\Maintenance\UpdateMaintenanceWorkOrderRequest;
use App\Http\Requests\Maintenance\SearchMaintenanceWorkOrderRequest;
use App\Http\Requests\Maintenance\BulkMaintenanceWorkOrderRequest;
...
{
    public function __construct(private MaintenanceWorkOrderService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchMaintenanceWorkOrderRequest $request): JsonResponse
    {
    ...
    public function store(StoreMaintenanceWorkOrderRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'MaintenanceWorkOrder created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'MaintenanceWorkOrder not found'], 404);
        }
    ...
    public function update(UpdateMaintenanceWorkOrderRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'MaintenanceWorkOrder not found'], 404);
        ...
        return response()->json([
            'message' => 'MaintenanceWorkOrder updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'MaintenanceWorkOrder deleted successfully']);
    }
    ...
    public function bulk(BulkMaintenanceWorkOrderRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('MaintenanceWorkOrder external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchMaintenanceWorkOrderRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
