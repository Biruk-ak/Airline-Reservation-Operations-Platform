...
namespace App\Http\Controllers\Api\Boarding;

use App\Http\Controllers\Controller;
use App\Models\Boarding\BoardingPass;
use App\Services\Boarding\BoardingPassService;
use App\Http\Requests\Boarding\StoreBoardingPassRequest;
use App\Http\Requests\Boarding\UpdateBoardingPassRequest;
use App\Http\Requests\Boarding\SearchBoardingPassRequest;
use App\Http\Requests\Boarding\BulkBoardingPassRequest;
...
{
    public function __construct(private BoardingPassService $service)
    {
        $this->middleware('auth:sanctum');
        $this->middleware('airline.scope');
    }

    public function index(SearchBoardingPassRequest $request): JsonResponse
    {
    ...
    public function store(StoreBoardingPassRequest $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'BoardingPass created successfully',
    ...
    public function show(Request $request, int $id): JsonResponse
    {
        ...
        if (!$entity) {
            return response()->json(['message' => 'BoardingPass not found'], 404);
        }
    ...
    public function update(UpdateBoardingPassRequest $request, int $id): JsonResponse
    {
        $entity = $this->service->findForAirline($id, $request->attributes->get('airline_id'));
        if (!$entity) {
            return response()->json(['message' => 'BoardingPass not found'], 404);
        ...
        return response()->json([
            'message' => 'BoardingPass updated successfully',
    ...
    public function destroy(Request $request, int $id): JsonResponse
    {
        ...
        return response()->json(['message' => 'BoardingPass deleted successfully']);
    }
    ...
    public function bulk(BulkBoardingPassRequest $request): JsonResponse
    {
    ...
    public function syncExternal(Request $request, int $id): JsonResponse
    {
        ...
        Log::info('BoardingPass external sync', ['id' => $id, 'user' => $request->user()->id]);
        return response()->json(['data' => $synced->toOperationalSummary()]);
    }

    public function export(SearchBoardingPassRequest $request): JsonResponse
    {
        $filters = $request->validated();
        $filters['airline_id'] = $request->attributes->get('airline_id');
