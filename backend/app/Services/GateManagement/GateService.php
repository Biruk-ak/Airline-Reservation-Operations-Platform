...
class GateService
{
    ...
    {
        $query = Gate::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?Gate
    {
        return Gate::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): Gate
    {
        ...
        $this->notifier->notifyModuleEvent('GateManagement', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(Gate $entity, array $payload): Gate
    {
        ...
        $this->notifier->notifyModuleEvent('GateManagement', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(Gate $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('GateManagement', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = Gate::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:GateManagement:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = Gate::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(Gate $entity): array
    {
    ...
    public function cloneRecord(Gate $entity, int $userId, array $overrides = []): Gate
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('GateManagement', 0, 3));
    ...
    {
        Cache::forget("stats:GateManagement:{$airlineId}");
    }
}
