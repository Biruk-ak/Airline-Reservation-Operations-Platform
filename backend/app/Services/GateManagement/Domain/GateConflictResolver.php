...
class GateConflictResolver
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('GateConflictResolver.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateConflictResolver.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateConflictResolver.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateConflictResolver.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:GateManagement:GateConflictResolver:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for GateConflictResolver'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('GateConflictResolver.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
