...
class GateAssigner
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('GateAssigner.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GateAssigner.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.GateManagement.GateAssigner.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GateAssigner.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.GateManagement.GateAssigner.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:GateManagement:GateAssigner:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for GateAssigner'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('GateAssigner.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
