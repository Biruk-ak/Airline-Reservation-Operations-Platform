...
class SeatBlockManager
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('SeatBlockManager.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SeatBlockManager.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SeatBlockManager.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SeatBlockManager.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:SeatSelection:SeatBlockManager:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for SeatBlockManager'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('SeatBlockManager.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
