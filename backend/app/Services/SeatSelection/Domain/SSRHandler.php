...
class SSRHandler
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('SSRHandler.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SSRHandler.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.SeatSelection.SSRHandler.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SSRHandler.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.SeatSelection.SSRHandler.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:SeatSelection:SSRHandler:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for SSRHandler'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('SSRHandler.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
