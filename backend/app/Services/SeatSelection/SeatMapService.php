...
class SeatMapService
{
    ...
    {
        $query = SeatMap::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?SeatMap
    {
        return SeatMap::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): SeatMap
    {
        ...
        $this->notifier->notifyModuleEvent('SeatSelection', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(SeatMap $entity, array $payload): SeatMap
    {
        ...
        $this->notifier->notifyModuleEvent('SeatSelection', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(SeatMap $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('SeatSelection', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = SeatMap::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:SeatSelection:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = SeatMap::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(SeatMap $entity): array
    {
    ...
    public function cloneRecord(SeatMap $entity, int $userId, array $overrides = []): SeatMap
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('SeatSelection', 0, 3));
    ...
    {
        Cache::forget("stats:SeatSelection:{$airlineId}");
    }
}
