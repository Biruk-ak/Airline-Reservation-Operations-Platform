...
/**
 * Domain service: PositionIngestor
...
class PositionIngestor
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('PositionIngestor.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('PositionIngestor.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('PositionIngestor.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.PositionIngestor.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:FlightTracking:PositionIngestor:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for PositionIngestor'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('PositionIngestor.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
