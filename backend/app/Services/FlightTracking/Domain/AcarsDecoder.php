...
class AcarsDecoder
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('AcarsDecoder.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AcarsDecoder.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AcarsDecoder.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightTracking.AcarsDecoder.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:FlightTracking:AcarsDecoder:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for AcarsDecoder'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('AcarsDecoder.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
