...
class FlightPositionService
{
    ...
    {
        $query = FlightPosition::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?FlightPosition
    {
        return FlightPosition::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): FlightPosition
    {
        ...
        $entity = FlightPosition::create($payload);
        ...
        $this->notifier->notifyModuleEvent('FlightTracking', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(FlightPosition $entity, array $payload): FlightPosition
    {
        ...
        $this->notifier->notifyModuleEvent('FlightTracking', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(FlightPosition $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('FlightTracking', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = FlightPosition::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:FlightTracking:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = FlightPosition::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(FlightPosition $entity): array
    {
    ...
    public function cloneRecord(FlightPosition $entity, int $userId, array $overrides = []): FlightPosition
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('FlightTracking', 0, 3));
    ...
    {
        Cache::forget("stats:FlightTracking:{$airlineId}");
    }
}
