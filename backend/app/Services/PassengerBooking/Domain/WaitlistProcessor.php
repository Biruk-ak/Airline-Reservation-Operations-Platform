...
/**
 * Domain service: WaitlistProcessor
...
class WaitlistProcessor
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('WaitlistProcessor.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WaitlistProcessor.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WaitlistProcessor.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.WaitlistProcessor.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:PassengerBooking:WaitlistProcessor:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for WaitlistProcessor'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('WaitlistProcessor.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
