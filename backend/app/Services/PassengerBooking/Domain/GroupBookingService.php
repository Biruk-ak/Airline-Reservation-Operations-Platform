...
namespace App\Services\PassengerBooking\Domain;
...
/**
 * Domain service: GroupBookingService
 * Module: PassengerBooking
 *
 * Encapsulates operational workflows for airline PassengerBooking processes.
 */
class GroupBookingService
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('GroupBookingService.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('GroupBookingService.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('GroupBookingService.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.GroupBookingService.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:PassengerBooking:GroupBookingService:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for GroupBookingService'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('GroupBookingService.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
