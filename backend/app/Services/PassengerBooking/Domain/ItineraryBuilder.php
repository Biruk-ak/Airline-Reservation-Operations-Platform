...
/**
 * Domain service: ItineraryBuilder
...
class ItineraryBuilder
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('ItineraryBuilder.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ItineraryBuilder.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ItineraryBuilder.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.PassengerBooking.ItineraryBuilder.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:PassengerBooking:ItineraryBuilder:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for ItineraryBuilder'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('ItineraryBuilder.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
