...
class FinalCloseoutService
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('FinalCloseoutService.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FinalCloseoutService.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FinalCloseoutService.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Boarding.FinalCloseoutService.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Boarding:FinalCloseoutService:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for FinalCloseoutService'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('FinalCloseoutService.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
