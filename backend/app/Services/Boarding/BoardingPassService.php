...
class BoardingPassService
{
    ...
    {
        $query = BoardingPass::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?BoardingPass
    {
        return BoardingPass::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): BoardingPass
    {
        ...
        $entity = BoardingPass::create($payload);
        ...
        $this->notifier->notifyModuleEvent('Boarding', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(BoardingPass $entity, array $payload): BoardingPass
    {
        ...
        $this->notifier->notifyModuleEvent('Boarding', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(BoardingPass $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('Boarding', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = BoardingPass::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:Boarding:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = BoardingPass::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(BoardingPass $entity): array
    {
    ...
    public function cloneRecord(BoardingPass $entity, int $userId, array $overrides = []): BoardingPass
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('Boarding', 0, 3));
    ...
    {
        Cache::forget("stats:Boarding:{$airlineId}");
    }
}
