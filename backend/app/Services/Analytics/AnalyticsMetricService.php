...
class AnalyticsMetricService
{
    ...
    {
        $query = AnalyticsMetric::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?AnalyticsMetric
    {
        return AnalyticsMetric::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): AnalyticsMetric
    {
        ...
        $entity = AnalyticsMetric::create($payload);
        ...
        $this->notifier->notifyModuleEvent('Analytics', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(AnalyticsMetric $entity, array $payload): AnalyticsMetric
    {
        ...
        $this->notifier->notifyModuleEvent('Analytics', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(AnalyticsMetric $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('Analytics', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = AnalyticsMetric::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:Analytics:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = AnalyticsMetric::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(AnalyticsMetric $entity): array
    {
    ...
    public function cloneRecord(AnalyticsMetric $entity, int $userId, array $overrides = []): AnalyticsMetric
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('Analytics', 0, 3));
    ...
    {
        Cache::forget("stats:Analytics:{$airlineId}");
    }
}
