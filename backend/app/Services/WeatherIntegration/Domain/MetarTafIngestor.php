...
class MetarTafIngestor
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('MetarTafIngestor.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('MetarTafIngestor.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('MetarTafIngestor.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.MetarTafIngestor.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:WeatherIntegration:MetarTafIngestor:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for MetarTafIngestor'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('MetarTafIngestor.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
