...
class SigmetAlertService
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('SigmetAlertService.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SigmetAlertService.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SigmetAlertService.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.WeatherIntegration.SigmetAlertService.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:WeatherIntegration:SigmetAlertService:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for SigmetAlertService'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('SigmetAlertService.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
