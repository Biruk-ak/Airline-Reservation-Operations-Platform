...
class WeatherObservationService
{
    ...
    {
        $query = WeatherObservation::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?WeatherObservation
    {
        return WeatherObservation::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): WeatherObservation
    {
        ...
        $entity = WeatherObservation::create($payload);
        ...
        $this->notifier->notifyModuleEvent('WeatherIntegration', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(WeatherObservation $entity, array $payload): WeatherObservation
    {
        ...
        $this->notifier->notifyModuleEvent('WeatherIntegration', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(WeatherObservation $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('WeatherIntegration', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = WeatherObservation::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:WeatherIntegration:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = WeatherObservation::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(WeatherObservation $entity): array
    {
    ...
    public function cloneRecord(WeatherObservation $entity, int $userId, array $overrides = []): WeatherObservation
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('WeatherIntegration', 0, 3));
    ...
    {
        Cache::forget("stats:WeatherIntegration:{$airlineId}");
    }
}
