...
/**
 * Domain service: WorkOrderPlanner
 * Module: Maintenance
 *
...
class WorkOrderPlanner
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('WorkOrderPlanner.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('WorkOrderPlanner.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('WorkOrderPlanner.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.WorkOrderPlanner.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Maintenance:WorkOrderPlanner:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for WorkOrderPlanner'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('WorkOrderPlanner.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
