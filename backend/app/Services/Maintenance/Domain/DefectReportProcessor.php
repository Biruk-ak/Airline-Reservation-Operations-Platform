...
class DefectReportProcessor
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('DefectReportProcessor.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DefectReportProcessor.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DefectReportProcessor.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Maintenance.DefectReportProcessor.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Maintenance:DefectReportProcessor:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for DefectReportProcessor'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('DefectReportProcessor.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
