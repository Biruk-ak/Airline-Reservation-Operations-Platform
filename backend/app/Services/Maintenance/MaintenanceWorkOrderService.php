...
class MaintenanceWorkOrderService
{
    ...
    {
        $query = MaintenanceWorkOrder::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?MaintenanceWorkOrder
    {
        return MaintenanceWorkOrder::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): MaintenanceWorkOrder
    {
        ...
        $entity = MaintenanceWorkOrder::create($payload);
        ...
        $this->notifier->notifyModuleEvent('Maintenance', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(MaintenanceWorkOrder $entity, array $payload): MaintenanceWorkOrder
    {
        ...
        $this->notifier->notifyModuleEvent('Maintenance', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(MaintenanceWorkOrder $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('Maintenance', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = MaintenanceWorkOrder::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:Maintenance:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = MaintenanceWorkOrder::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(MaintenanceWorkOrder $entity): array
    {
    ...
    public function cloneRecord(MaintenanceWorkOrder $entity, int $userId, array $overrides = []): MaintenanceWorkOrder
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('Maintenance', 0, 3));
    ...
    {
        Cache::forget("stats:Maintenance:{$airlineId}");
    }
}
