...
class FareRuleService
{
    ...
    {
        $query = FareRule::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?FareRule
    {
        return FareRule::query()->forAirline($airlineId)->whereKey($id)->first();
    }
    ...
    public function create(array $payload): FareRule
    {
        ...
        $entity = FareRule::create($payload);
        ...
        $this->notifier->notifyModuleEvent('TicketPricing', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(FareRule $entity, array $payload): FareRule
    {
        ...
        $this->notifier->notifyModuleEvent('TicketPricing', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(FareRule $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('TicketPricing', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = FareRule::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:TicketPricing:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = FareRule::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(FareRule $entity): array
    {
    ...
    public function cloneRecord(FareRule $entity, int $userId, array $overrides = []): FareRule
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('TicketPricing', 0, 3));
    ...
    {
        Cache::forget("stats:TicketPricing:{$airlineId}");
    }
}
