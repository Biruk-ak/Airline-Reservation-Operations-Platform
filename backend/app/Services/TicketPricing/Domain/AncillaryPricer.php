...
class AncillaryPricer
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('AncillaryPricer.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('AncillaryPricer.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('AncillaryPricer.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.TicketPricing.AncillaryPricer.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:TicketPricing:AncillaryPricer:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for AncillaryPricer'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('AncillaryPricer.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
