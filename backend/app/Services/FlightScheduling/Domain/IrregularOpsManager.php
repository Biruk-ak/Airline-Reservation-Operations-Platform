...
class IrregularOpsManager
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('IrregularOpsManager.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('IrregularOpsManager.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('IrregularOpsManager.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.IrregularOpsManager.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:FlightScheduling:IrregularOpsManager:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for IrregularOpsManager'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('IrregularOpsManager.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
