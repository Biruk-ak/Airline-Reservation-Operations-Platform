...
/**
 * Domain service: RouteNetworkService
...
class RouteNetworkService
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('RouteNetworkService.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RouteNetworkService.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RouteNetworkService.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.RouteNetworkService.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:FlightScheduling:RouteNetworkService:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for RouteNetworkService'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('RouteNetworkService.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
