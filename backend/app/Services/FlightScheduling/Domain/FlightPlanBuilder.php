...
/**
 * Domain service: FlightPlanBuilder
...
class FlightPlanBuilder
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('FlightPlanBuilder.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FlightPlanBuilder.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FlightPlanBuilder.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.FlightScheduling.FlightPlanBuilder.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:FlightScheduling:FlightPlanBuilder:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for FlightPlanBuilder'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('FlightPlanBuilder.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
