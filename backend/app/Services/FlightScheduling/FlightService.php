...
class FlightService
{
    ...
    {
        $query = Flight::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?Flight
    {
        return Flight::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): Flight
    {
        ...
        $this->notifier->notifyModuleEvent('FlightScheduling', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(Flight $entity, array $payload): Flight
    {
        ...
        $this->notifier->notifyModuleEvent('FlightScheduling', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(Flight $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('FlightScheduling', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = Flight::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:FlightScheduling:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = Flight::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(Flight $entity): array
    {
    ...
    public function cloneRecord(Flight $entity, int $userId, array $overrides = []): Flight
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('FlightScheduling', 0, 3));
    ...
    {
        Cache::forget("stats:FlightScheduling:{$airlineId}");
    }
}
