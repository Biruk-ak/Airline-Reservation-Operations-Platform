...
class ReportDefinitionService
{
    ...
    {
        $query = ReportDefinition::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?ReportDefinition
    {
        return ReportDefinition::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): ReportDefinition
    {
        ...
        $entity = ReportDefinition::create($payload);
        ...
        $this->notifier->notifyModuleEvent('Reporting', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(ReportDefinition $entity, array $payload): ReportDefinition
    {
        ...
        $this->notifier->notifyModuleEvent('Reporting', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(ReportDefinition $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('Reporting', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = ReportDefinition::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:Reporting:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = ReportDefinition::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(ReportDefinition $entity): array
    {
    ...
    public function cloneRecord(ReportDefinition $entity, int $userId, array $overrides = []): ReportDefinition
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('Reporting', 0, 3));
    ...
    {
        Cache::forget("stats:Reporting:{$airlineId}");
    }
}
