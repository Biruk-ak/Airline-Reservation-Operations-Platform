...
/**
 * Domain service: OperationalReportBuilder
 * Module: Reporting
 *
...
class OperationalReportBuilder
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('OperationalReportBuilder.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('OperationalReportBuilder.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('OperationalReportBuilder.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.OperationalReportBuilder.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Reporting:OperationalReportBuilder:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function compute(string $verb, array $context): array
    {
        ...
        return [
            'service' => 'OperationalReportBuilder',
            'module' => 'Reporting',
            'verb' => $verb,
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for OperationalReportBuilder'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('OperationalReportBuilder.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
