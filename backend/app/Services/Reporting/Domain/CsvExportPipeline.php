...
/**
 * Domain service: CsvExportPipeline
 * Module: Reporting
 *
...
class CsvExportPipeline
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('CsvExportPipeline.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CsvExportPipeline.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CsvExportPipeline.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Reporting.CsvExportPipeline.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Reporting:CsvExportPipeline:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for CsvExportPipeline'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('CsvExportPipeline.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
