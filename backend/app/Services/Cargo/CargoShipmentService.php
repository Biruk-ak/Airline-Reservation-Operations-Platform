...
class CargoShipmentService
{
    ...
    {
        $query = CargoShipment::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?CargoShipment
    {
        return CargoShipment::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): CargoShipment
    {
        ...
        $entity = CargoShipment::create($payload);
        ...
        $this->notifier->notifyModuleEvent('Cargo', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(CargoShipment $entity, array $payload): CargoShipment
    {
        ...
        $this->notifier->notifyModuleEvent('Cargo', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(CargoShipment $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('Cargo', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = CargoShipment::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:Cargo:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = CargoShipment::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(CargoShipment $entity): array
    {
    ...
    public function cloneRecord(CargoShipment $entity, int $userId, array $overrides = []): CargoShipment
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('Cargo', 0, 3));
    ...
    {
        Cache::forget("stats:Cargo:{$airlineId}");
    }
}
