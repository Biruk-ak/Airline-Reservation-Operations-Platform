...
class CargoBookingEngine
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('CargoBookingEngine.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoBookingEngine.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoBookingEngine.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoBookingEngine.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Cargo:CargoBookingEngine:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for CargoBookingEngine'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('CargoBookingEngine.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
