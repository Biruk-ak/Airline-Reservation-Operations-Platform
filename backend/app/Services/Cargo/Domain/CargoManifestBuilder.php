...
class CargoManifestBuilder
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('CargoManifestBuilder.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CargoManifestBuilder.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CargoManifestBuilder.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Cargo.CargoManifestBuilder.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Cargo:CargoManifestBuilder:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for CargoManifestBuilder'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('CargoManifestBuilder.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
