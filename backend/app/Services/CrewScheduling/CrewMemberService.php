...
class CrewMemberService
{
    ...
    {
        $query = CrewMember::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?CrewMember
    {
        return CrewMember::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): CrewMember
    {
        ...
        $entity = CrewMember::create($payload);
        ...
        $this->notifier->notifyModuleEvent('CrewScheduling', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(CrewMember $entity, array $payload): CrewMember
    {
        ...
        $this->notifier->notifyModuleEvent('CrewScheduling', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(CrewMember $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('CrewScheduling', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = CrewMember::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:CrewScheduling:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = CrewMember::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(CrewMember $entity): array
    {
    ...
    public function cloneRecord(CrewMember $entity, int $userId, array $overrides = []): CrewMember
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('CrewScheduling', 0, 3));
    ...
    {
        Cache::forget("stats:CrewScheduling:{$airlineId}");
    }
}
