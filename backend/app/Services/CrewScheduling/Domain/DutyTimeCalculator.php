...
/**
 * Domain service: DutyTimeCalculator
...
class DutyTimeCalculator
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('DutyTimeCalculator.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('DutyTimeCalculator.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('DutyTimeCalculator.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.DutyTimeCalculator.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:CrewScheduling:DutyTimeCalculator:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for DutyTimeCalculator'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('DutyTimeCalculator.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
