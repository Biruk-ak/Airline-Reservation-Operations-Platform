...
class RosterBuilder
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('RosterBuilder.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RosterBuilder.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RosterBuilder.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CrewScheduling.RosterBuilder.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:CrewScheduling:RosterBuilder:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for RosterBuilder'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('RosterBuilder.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
