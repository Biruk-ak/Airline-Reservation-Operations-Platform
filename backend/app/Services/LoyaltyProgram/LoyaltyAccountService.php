...
class LoyaltyAccountService
{
    ...
    {
        $query = LoyaltyAccount::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?LoyaltyAccount
    {
        return LoyaltyAccount::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): LoyaltyAccount
    {
        ...
        $entity = LoyaltyAccount::create($payload);
        ...
        $this->notifier->notifyModuleEvent('LoyaltyProgram', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(LoyaltyAccount $entity, array $payload): LoyaltyAccount
    {
        ...
        $this->notifier->notifyModuleEvent('LoyaltyProgram', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(LoyaltyAccount $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('LoyaltyProgram', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = LoyaltyAccount::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:LoyaltyProgram:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = LoyaltyAccount::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(LoyaltyAccount $entity): array
    {
    ...
    public function cloneRecord(LoyaltyAccount $entity, int $userId, array $overrides = []): LoyaltyAccount
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('LoyaltyProgram', 0, 3));
    ...
    {
        Cache::forget("stats:LoyaltyProgram:{$airlineId}");
    }
}
