...
namespace App\Services\LoyaltyProgram\Domain;
...
/**
 * Domain service: ExpirationPolicyRunner
 * Module: LoyaltyProgram
 *
 * Encapsulates operational workflows for airline LoyaltyProgram processes.
 */
class ExpirationPolicyRunner
{
    protected $logger;
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('ExpirationPolicyRunner.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ExpirationPolicyRunner.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ExpirationPolicyRunner.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.ExpirationPolicyRunner.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:LoyaltyProgram:ExpirationPolicyRunner:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function compute(string $verb, array $context): array
    {
        ...
        return [
            'service' => 'ExpirationPolicyRunner',
            'module' => 'LoyaltyProgram',
            'verb' => $verb,
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for ExpirationPolicyRunner'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('ExpirationPolicyRunner.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
