...
class TierEvaluator
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('TierEvaluator.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('TierEvaluator.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('TierEvaluator.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.TierEvaluator.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:LoyaltyProgram:TierEvaluator:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for TierEvaluator'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('TierEvaluator.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
