...
/**
 * Domain service: RedemptionValidator
...
class RedemptionValidator
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('RedemptionValidator.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('RedemptionValidator.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('RedemptionValidator.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.LoyaltyProgram.RedemptionValidator.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:LoyaltyProgram:RedemptionValidator:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for RedemptionValidator'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('RedemptionValidator.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
