...
class AdminAuditLogService
{
    ...
    {
        $query = AdminAuditLog::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?AdminAuditLog
    {
        return AdminAuditLog::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): AdminAuditLog
    {
        ...
        $entity = AdminAuditLog::create($payload);
        ...
        $this->notifier->notifyModuleEvent('AdminPortal', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(AdminAuditLog $entity, array $payload): AdminAuditLog
    {
        ...
        $this->notifier->notifyModuleEvent('AdminPortal', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(AdminAuditLog $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('AdminPortal', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = AdminAuditLog::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:AdminPortal:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = AdminAuditLog::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(AdminAuditLog $entity): array
    {
    ...
    public function cloneRecord(AdminAuditLog $entity, int $userId, array $overrides = []): AdminAuditLog
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('AdminPortal', 0, 3));
    ...
    {
        Cache::forget("stats:AdminPortal:{$airlineId}");
    }
}
