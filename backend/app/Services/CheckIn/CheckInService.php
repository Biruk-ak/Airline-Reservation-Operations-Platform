...
class CheckInService
{
    ...
    {
        $query = CheckIn::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?CheckIn
    {
        return CheckIn::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): CheckIn
    {
        ...
        $this->notifier->notifyModuleEvent('CheckIn', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(CheckIn $entity, array $payload): CheckIn
    {
        ...
        $this->notifier->notifyModuleEvent('CheckIn', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(CheckIn $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('CheckIn', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = CheckIn::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:CheckIn:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = CheckIn::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(CheckIn $entity): array
    {
    ...
    public function cloneRecord(CheckIn $entity, int $userId, array $overrides = []): CheckIn
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('CheckIn', 0, 3));
    ...
    {
        Cache::forget("stats:CheckIn:{$airlineId}");
    }
}
