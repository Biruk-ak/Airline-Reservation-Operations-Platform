...
class BoardingPassIssuer
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('BoardingPassIssuer.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('BoardingPassIssuer.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('BoardingPassIssuer.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.CheckIn.BoardingPassIssuer.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:CheckIn:BoardingPassIssuer:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for BoardingPassIssuer'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('BoardingPassIssuer.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
