...
namespace App\Services\BaggageTracking;
...
class BaggageItemService
{
    ...
    {
        $query = BaggageItem::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?BaggageItem
    {
        return BaggageItem::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): BaggageItem
    {
        ...
        $entity = BaggageItem::create($payload);
        ...
        $this->notifier->notifyModuleEvent('BaggageTracking', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(BaggageItem $entity, array $payload): BaggageItem
    {
        ...
        $this->notifier->notifyModuleEvent('BaggageTracking', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(BaggageItem $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('BaggageTracking', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = BaggageItem::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:BaggageTracking:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = BaggageItem::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(BaggageItem $entity): array
    {
    ...
    public function cloneRecord(BaggageItem $entity, int $userId, array $overrides = []): BaggageItem
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('BaggageTracking', 0, 3));
    ...
    {
        Cache::forget("stats:BaggageTracking:{$airlineId}");
    }
}
