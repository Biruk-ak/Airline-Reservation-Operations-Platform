...
/**
 * Domain service: CurrencyConverter
 * Module: Payments
 *
...
class CurrencyConverter
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('CurrencyConverter.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('CurrencyConverter.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.CurrencyConverter.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('CurrencyConverter.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.CurrencyConverter.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Payments:CurrencyConverter:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for CurrencyConverter'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('CurrencyConverter.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
