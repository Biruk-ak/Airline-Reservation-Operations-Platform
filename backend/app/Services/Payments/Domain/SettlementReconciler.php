...
/**
 * Domain service: SettlementReconciler
 * Module: Payments
 *
...
class SettlementReconciler
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('SettlementReconciler.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SettlementReconciler.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.SettlementReconciler.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SettlementReconciler.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.SettlementReconciler.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Payments:SettlementReconciler:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for SettlementReconciler'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('SettlementReconciler.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
