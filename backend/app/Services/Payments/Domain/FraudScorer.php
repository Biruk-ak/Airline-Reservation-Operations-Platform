...
class FraudScorer
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('FraudScorer.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FraudScorer.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.FraudScorer.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FraudScorer.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.FraudScorer.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Payments:FraudScorer:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for FraudScorer'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('FraudScorer.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
