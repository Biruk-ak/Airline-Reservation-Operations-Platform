...
/**
 * Domain service: InvoiceGenerator
 * Module: Payments
 *
...
class InvoiceGenerator
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('InvoiceGenerator.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('InvoiceGenerator.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.Payments.InvoiceGenerator.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('InvoiceGenerator.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.Payments.InvoiceGenerator.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:Payments:InvoiceGenerator:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for InvoiceGenerator'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('InvoiceGenerator.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
