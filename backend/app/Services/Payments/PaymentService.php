...
class PaymentService
{
    ...
    {
        $query = Payment::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?Payment
    {
        return Payment::query()->forAirline($airlineId)->whereKey($id)->first();
    }
    ...
    public function create(array $payload): Payment
    {
        ...
        $this->notifier->notifyModuleEvent('Payments', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(Payment $entity, array $payload): Payment
    {
        ...
        $this->notifier->notifyModuleEvent('Payments', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(Payment $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('Payments', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = Payment::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:Payments:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = Payment::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(Payment $entity): array
    {
    ...
    public function cloneRecord(Payment $entity, int $userId, array $overrides = []): Payment
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('Payments', 0, 3));
    ...
    {
        Cache::forget("stats:Payments:{$airlineId}");
    }
}
