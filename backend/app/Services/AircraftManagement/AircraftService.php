...
class AircraftService
{
    ...
    {
        $query = Aircraft::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?Aircraft
    {
        return Aircraft::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): Aircraft
    {
        ...
        $entity = Aircraft::create($payload);
        ...
        $this->notifier->notifyModuleEvent('AircraftManagement', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(Aircraft $entity, array $payload): Aircraft
    {
        ...
        $this->notifier->notifyModuleEvent('AircraftManagement', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(Aircraft $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('AircraftManagement', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = Aircraft::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:AircraftManagement:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = Aircraft::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(Aircraft $entity): array
    {
    ...
    public function cloneRecord(Aircraft $entity, int $userId, array $overrides = []): Aircraft
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('AircraftManagement', 0, 3));
    ...
    {
        Cache::forget("stats:AircraftManagement:{$airlineId}");
    }
}
