...
/**
 * Domain service: UtilizationAnalyzer
...
class UtilizationAnalyzer
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('UtilizationAnalyzer.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('UtilizationAnalyzer.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('UtilizationAnalyzer.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.UtilizationAnalyzer.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:AircraftManagement:UtilizationAnalyzer:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for UtilizationAnalyzer'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('UtilizationAnalyzer.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
