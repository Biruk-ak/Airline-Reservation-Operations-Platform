...
/**
 * Domain service: LeaseContractService
...
class LeaseContractService
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('LeaseContractService.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('LeaseContractService.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('LeaseContractService.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.LeaseContractService.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:AircraftManagement:LeaseContractService:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for LeaseContractService'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('LeaseContractService.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
