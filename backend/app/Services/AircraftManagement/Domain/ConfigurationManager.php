...
/**
 * Domain service: ConfigurationManager
...
class ConfigurationManager
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('ConfigurationManager.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ConfigurationManager.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ConfigurationManager.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.ConfigurationManager.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:AircraftManagement:ConfigurationManager:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for ConfigurationManager'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('ConfigurationManager.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
