...
/**
 * Domain service: FleetAssignmentEngine
...
class FleetAssignmentEngine
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('FleetAssignmentEngine.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('FleetAssignmentEngine.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('FleetAssignmentEngine.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AircraftManagement.FleetAssignmentEngine.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:AircraftManagement:FleetAssignmentEngine:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for FleetAssignmentEngine'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('FleetAssignmentEngine.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
