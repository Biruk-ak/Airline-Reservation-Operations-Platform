...
class AirportService
{
    ...
    {
        $query = Airport::query()->forAirline($filters['airline_id'] ?? 0);
    ...
    public function findForAirline(int $id, int $airlineId): ?Airport
    {
        return Airport::query()->forAirline($airlineId)->whereKey($id)->first();
    }

    public function create(array $payload): Airport
    {
        ...
        $this->notifier->notifyModuleEvent('AirportOperations', 'created', $entity->toOperationalSummary());
        $this->bustCache($payload['airline_id'] ?? 0);

        return $entity->fresh();
    }

    public function update(Airport $entity, array $payload): Airport
    {
        ...
        $this->notifier->notifyModuleEvent('AirportOperations', 'updated', $entity->toOperationalSummary());
        $this->bustCache($entity->airline_id);
        return $entity->fresh();
    }

    public function delete(Airport $entity, int $userId): void
    {
        ...
        $this->notifier->notifyModuleEvent('AirportOperations', 'deleted', $summary);
        $this->bustCache($entity->airline_id);
    }
    ...
    public function bulkAction(string $action, array $ids, int $airlineId, int $userId, array $payload = []): array
    {
        $affected = 0;
        $errors = [];
        $items = Airport::query()->forAirline($airlineId)->whereIn('id', $ids)->get();
    ...
    {
        $cacheKey = "stats:AirportOperations:{$airlineId}";
        return Cache::remember($cacheKey, 120, function () use ($airlineId) {
            $base = Airport::query()->forAirline($airlineId);
            return [
    ...
    public function timeline(Airport $entity): array
    {
    ...
    public function cloneRecord(Airport $entity, int $userId, array $overrides = []): Airport
    {
        $data = $entity->only([
    ...
    {
        $prefix = strtoupper(substr('AirportOperations', 0, 3));
    ...
    {
        Cache::forget("stats:AirportOperations:{$airlineId}");
    }
}
