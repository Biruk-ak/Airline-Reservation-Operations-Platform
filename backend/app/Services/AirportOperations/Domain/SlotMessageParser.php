...
class SlotMessageParser
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('SlotMessageParser.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('SlotMessageParser.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('SlotMessageParser.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.SlotMessageParser.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:AirportOperations:SlotMessageParser:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for SlotMessageParser'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('SlotMessageParser.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
