...
class StationOpsDashboard
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('StationOpsDashboard.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('StationOpsDashboard.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('StationOpsDashboard.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.StationOpsDashboard.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:AirportOperations:StationOpsDashboard:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for StationOpsDashboard'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('StationOpsDashboard.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
