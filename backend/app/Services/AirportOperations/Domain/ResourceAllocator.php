...
class ResourceAllocator
{
    ...
    public function execute(array $context = []): array
    {
        ...
        $this->logger->info('ResourceAllocator.execute.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.execute.success');

            return $this->success('execute', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.execute.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.execute.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.validate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.validate.success');

            return $this->success('validate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.validate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.validate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.simulate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.simulate.success');

            return $this->success('simulate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.simulate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.simulate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.reconcile.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.reconcile.success');

            return $this->success('reconcile', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.reconcile.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.reconcile.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.optimize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.optimize.success');

            return $this->success('optimize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.optimize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.optimize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.forecast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.forecast.success');

            return $this->success('forecast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.forecast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.forecast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.publish.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.publish.success');

            return $this->success('publish', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.publish.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.publish.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.rollback.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.rollback.success');

            return $this->success('rollback', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.rollback.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.rollback.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.snapshot.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.snapshot.success');

            return $this->success('snapshot', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.snapshot.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.snapshot.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.diff.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.diff.success');

            return $this->success('diff', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.diff.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.diff.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.enrich.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.enrich.success');

            return $this->success('enrich', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.enrich.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.enrich.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.normalize.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.normalize.success');

            return $this->success('normalize', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.normalize.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.normalize.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.aggregate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.aggregate.success');

            return $this->success('aggregate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.aggregate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.aggregate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.dispatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.dispatch.success');

            return $this->success('dispatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.dispatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.dispatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.acknowledge.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.acknowledge.success');

            return $this->success('acknowledge', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.acknowledge.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.acknowledge.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.escalate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.escalate.success');

            return $this->success('escalate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.escalate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.escalate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.suppress.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.suppress.success');

            return $this->success('suppress', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.suppress.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.suppress.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.replay.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.replay.success');

            return $this->success('replay', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.replay.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.replay.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.hydrate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.hydrate.success');

            return $this->success('hydrate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.hydrate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.hydrate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.persist.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.persist.success');

            return $this->success('persist', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.persist.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.persist.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.invalidate.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.invalidate.success');

            return $this->success('invalidate', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.invalidate.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.invalidate.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.broadcast.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.broadcast.success');

            return $this->success('broadcast', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.broadcast.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.broadcast.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.compact.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.compact.success');

            return $this->success('compact', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.compact.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.compact.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.archiveBatch.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.archiveBatch.success');

            return $this->success('archiveBatch', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.archiveBatch.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.archiveBatch.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.evaluateRules.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.evaluateRules.success');

            return $this->success('evaluateRules', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.evaluateRules.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.evaluateRules.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.applyPolicy.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.applyPolicy.success');

            return $this->success('applyPolicy', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.applyPolicy.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.applyPolicy.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.computeMetrics.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.computeMetrics.success');

            return $this->success('computeMetrics', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.computeMetrics.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.computeMetrics.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.buildPayload.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.buildPayload.success');

            return $this->success('buildPayload', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.buildPayload.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.buildPayload.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.transformInput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.transformInput.success');

            return $this->success('transformInput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.transformInput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.transformInput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.transformOutput.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.transformOutput.success');

            return $this->success('transformOutput', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.transformOutput.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.transformOutput.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.checkPreconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.checkPreconditions.success');

            return $this->success('checkPreconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.checkPreconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.checkPreconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.checkPostconditions.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.checkPostconditions.success');

            return $this->success('checkPostconditions', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.checkPostconditions.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.checkPostconditions.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.reserveResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.reserveResources.success');

            return $this->success('reserveResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.reserveResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.reserveResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.releaseResources.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.releaseResources.success');

            return $this->success('releaseResources', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.releaseResources.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.releaseResources.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.lockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.lockEntity.success');

            return $this->success('lockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.lockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.lockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.unlockEntity.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.unlockEntity.success');

            return $this->success('unlockEntity', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.unlockEntity.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.unlockEntity.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.queueFollowUp.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.queueFollowUp.success');

            return $this->success('queueFollowUp', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.queueFollowUp.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.queueFollowUp.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.scheduleRetry.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.scheduleRetry.success');

            return $this->success('scheduleRetry', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.scheduleRetry.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.scheduleRetry.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.markComplete.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.markComplete.success');

            return $this->success('markComplete', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.markComplete.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.markComplete.failure');
    ...
    {
        $startedAt = microtime(true);
        $this->logger->info('ResourceAllocator.markFailed.start', ['context_keys' => array_keys($context)]);
        ...
        try {
            ...
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.markFailed.success');

            return $this->success('markFailed', $result, $startedAt);
        } catch (\Throwable $e) {
            $this->logger->error('ResourceAllocator.markFailed.error', [
                'message' => $e->getMessage(),
                'trace' => collect($e->getTrace())->take(5)->all(),
            ]);
            $this->metrics->increment('domain.AirportOperations.ResourceAllocator.markFailed.failure');
    ...
    {
        $cacheKey = sprintf('domain:AirportOperations:ResourceAllocator:%s:%s', $verb, md5(json_encode($context['payload'])));
    ...
    protected function buildRecommendations(string $verb, float $score, array $payload): array
    {
        $recs = [];
        if ($score > 100) {
            $recs[] = ['code' => 'HIGH_LOAD', 'message' => 'High operational load detected for ResourceAllocator'];
        }
    ...
    protected function afterHook(string $verb, array $context, array $result): void
    {
        if (in_array($verb, ['publish', 'broadcast', 'escalate'], true)) {
            $this->logger->info('ResourceAllocator.after.'.$verb, [
                'correlation_id' => $context['correlation_id'],
